/**
 * Simple SQLite Database Queries for ChosenWorship
 * SQLite-only implementation for development
 */
...
export async function getAllSetlists(): Promise<Setlist[]> {
  const setlists = await db.select().from(schema.setlists).orderBy(desc(schema.setlists.date));
...
export async function getSetlistById(id: string): Promise<Setlist | null> {
  const [setlist] = await db.select().from(schema.setlists).where(eq(schema.setlists.id, id));
...
async function transformSetlistFromDb(dbSetlist: any): Promise<Setlist> {
  // Get setlist songs
  const dbSetlistSongs = await db.select().from(schema.setlistSongs)
    .where(eq(schema.setlistSongs.setlistId, dbSetlist.id))
    .orderBy(asc(schema.setlistSongs.order));

  const setlistSongs = dbSetlistSongs.map((song: any) => ({
    songId: song.songId,
